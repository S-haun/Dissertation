...
NUMBAT23_sat_flows_brief = flows_brief(NUMBAT23_sat)
NUMBAT22_sat_flows_brief = flows_brief(NUMBAT22_sat)
NUMBAT19_sat_flows_brief = flows_brief(NUMBAT19_sat)
NUMBAT18_sat_flows_brief = flows_brief(NUMBAT18_sat)
NUMBAT17_sat_flows_brief = flows_brief(NUMBAT17_sat)
NUMBAT16_sat_flows_brief = flows_brief(NUMBAT16_sat)
...
def process_batch(links_batch, seq_length=1):
    ...
    for link in links_batch:
        ...
        for year, data in zip([2016, 2017, 2018, 2019, 2022], 
                              [NUMBAT16_sat_flows_brief, NUMBAT17_sat_flows_brief, NUMBAT18_sat_flows_brief, NUMBAT19_sat_flows_brief, NUMBAT22_sat_flows_brief]):
...
def plot_predictions(predictions, actual_data, historical_data, selected_links):
    ...
    for link in selected_links:
        ...
        plt.title(f'Typical Saturday, Link: {link}')
        plt.xlabel('Year')
        plt.ylabel('Load')
        plt.legend()
        plt.grid(True)
...
def plot_predictions2(predictions, selected_links):
    ...
    plt.title("Predictions for 10 Selected Links Across Time Periods Saturday")
...
if __name__ == "__main__":
    ...
    common_links = set(NUMBAT16_sat_flows_brief['Link']).intersection(
        NUMBAT17_sat_flows_brief['Link'],
        NUMBAT18_sat_flows_brief['Link'],
        NUMBAT19_sat_flows_brief['Link'],
        NUMBAT22_sat_flows_brief['Link'],
        NUMBAT23_sat_flows_brief['Link']
    )

    # Run the parallel processing for all common links
    all_predictions2023_sat = parallel_process(list(common_links), num_processes=4, seq_length=1)
    print("final sun prediction is: ")
    print(all_predictions2023_sat)

    selected_links = random.sample(list(all_predictions2023_sat.keys()), 10)

    # Prepare historical data in a dictionary
    historical_data = {
        2016: NUMBAT16_sat_flows_brief,
        2017: NUMBAT17_sat_flows_brief,
        2018: NUMBAT18_sat_flows_brief,
        2019: NUMBAT19_sat_flows_brief,
        2022: NUMBAT22_sat_flows_brief
    }

    # Plot predictions vs actual and historical data
    plot_predictions(all_predictions2023_sat, NUMBAT23_sat_flows_brief, historical_data, selected_links)

    plot_predictions2(all_predictions2023_sat, selected_links)

    # Prepare time period data for the selected links
    time_period_data = prepare_time_period_data(all_predictions2023_sat, selected_links)
    ...
    plt.title("Correlation Matrix for Satday Selected Links")
    plt.show()

    # Prepare data for all predictions
    all_predictions_df = prepare_all_predictions_data(all_predictions2023_sat)
    ...
    plot_correlation_matrix(correlation_matrix_all, "Correlation Matrix for 50 Saturday Predictions Across Links")
