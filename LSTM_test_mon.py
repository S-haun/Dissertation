...
NUMBAT23_mon_flows_brief = flows_brief(NUMBAT23_mon)
NUMBAT22_mon_flows_brief = flows_brief(NUMBAT22_mon)
NUMBAT19_mtt_flows_brief = flows_brief(NUMBAT19_mtt)
NUMBAT18_mtt_flows_brief = flows_brief(NUMBAT18_mtt)
NUMBAT17_mtt_flows_brief = flows_brief(NUMBAT17_mtt)
NUMBAT16_mtt_flows_brief = flows_brief(NUMBAT16_mtt)
...
def process_batch(links_batch, seq_length=1):
    ...
    for link in links_batch:
        ...
        for year, data in zip([2016, 2017, 2018, 2019, 2022], 
                              [NUMBAT16_mtt_flows_brief, NUMBAT17_mtt_flows_brief, NUMBAT18_mtt_flows_brief, NUMBAT19_mtt_flows_brief, NUMBAT22_mon_flows_brief]):
...
def plot_predictions(predictions, actual_data, historical_data, selected_links):
    ...
    for link in selected_links:
        ...
        plt.title(f'Typical weekday, Link: {link}')
        plt.xlabel('Year')
        plt.ylabel('Load')
        plt.legend()
        plt.grid(True)
...
def plot_predictions2(predictions, selected_links):
    ...
    plt.title("Predictions for 10 Selected Links Across Time Periods WEEKDAY")
...
if __name__ == "__main__":
    ...
    common_links = set(NUMBAT16_mtt_flows_brief['Link']).intersection(
        NUMBAT17_mtt_flows_brief['Link'],
        NUMBAT18_mtt_flows_brief['Link'],
        NUMBAT19_mtt_flows_brief['Link'],
        NUMBAT22_mon_flows_brief['Link'],
        NUMBAT23_mon_flows_brief['Link']
    )

    # Run the parallel processing for all common links
    all_predictions2023_mon = parallel_process(list(common_links), num_processes=4, seq_length=1)
    print("final weekday prediction is: ")
    print(all_predictions2023_mon)

    selected_links = random.sample(list(all_predictions2023_mon.keys()), 10)

    # Prepare historical data in a dictionary
    historical_data = {
        2016: NUMBAT16_mtt_flows_brief,
        2017: NUMBAT17_mtt_flows_brief,
        2018: NUMBAT18_mtt_flows_brief,
        2019: NUMBAT19_mtt_flows_brief,
        2022: NUMBAT22_mon_flows_brief
    }

    # Plot predictions vs actual and historical data
    plot_predictions(all_predictions2023_mon, NUMBAT23_mon_flows_brief, historical_data, selected_links)

    plot_predictions2(all_predictions2023_mon, selected_links)

    # Prepare time period data for the selected links
    time_period_data = prepare_time_period_data(all_predictions2023_mon, selected_links)
    ...
    plt.title("Correlation Matrix for weekday Selected Links")
    plt.show()

    # Prepare data for all predictions
    all_predictions_df = prepare_all_predictions_data(all_predictions2023_mon)
    ...
    plot_correlation_matrix(correlation_matrix_all, "Correlation Matrix for 50 Predictions Across Links WEEKDAY")
